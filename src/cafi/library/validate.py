...
_NON_ALPHA_CHARS = (
    set(string.digits) | set(_esc(string.punctuation)) | set(_esc(string.whitespace))
)
...
_NO_DIGITS_BAN: Final[set[str]] = {_CAT_DIGIT, _CAT_WORD} | set(string.digits)


def _check_prefix_suffix_constraints(
    pre_suf: str, constraints: set[str], bid: int, err_msg: str, /
) -> None:
...
def _check_regex(r_ccno: str, r_id: AcrCoreReg, bid: int, /) -> None:
    ...
    _check_prefix_suffix_constraints(
        r_id.pre,
        _WORDS_ONLY_BAN,
        bid,
